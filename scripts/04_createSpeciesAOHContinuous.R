...
# Changes: 
# (1) immediately upload to GDrive to save space
...
packages <- c("sf", "terra", "here", "rgrass", "dplyr", "data.table", 
              "patchwork", "tidyterra", "ggspatial", "prettymapr",
              "parallel", "ggplot2","googledrive")
...
createAOH <- function(spp_name, ranges, folder_name, ifComp, col_srtm, col_bound){
  outfolder <- 'data/col_aoh/2012_7gen_btst_keephab/'
  dir.create('data/col_aoh', showWarnings = FALSE)
  dir.create('data/col_aoh/2012_7gen_btst_keephab', showWarnings = FALSE)
  ...
  if(length(pref)== 0 | is.null(pref)){
    ...
    if(!file.exists('data/pref_needs_info_btstgen7keep.csv')){
      write.table(df, 'data/pref_needs_info_btstgen7keep.csv', sep = ';', row.names = FALSE)
    }else{
      write.table(df, 'data/pref_needs_info_btstgen7keep.csv', append=TRUE, col.names = FALSE,
                  row.names = FALSE, sep = ';')
    }
  } else{
    ...
    if(is.null(sp_range) | NA %in% st_is_valid(sp_range)){
      ...
      if(problem==0){
        ...
        for(i in 1:length(pref)){
          gc()
          print(pref[i])
          prj_path <- paste0('data/hab_layers/2012_btst_7gen_keephab/', pref[i], '_layer_prj.tif')
          if(!file.exists(prj_path)){
            prefi_dft <- rast(paste0('data/hab_layers/2012_btst_7gen_keephab/', pref[i], '_layer.tif'))
            prefi_prj <- project(prefi_dft, "EPSG:4326", method = "near")
            writeRaster(prefi_prj, prj_path, datatype = "INT2U")
            rm(prefi_dft)
            rm(prefi_prj)
            gc()
          }
        ...
        writeRaster(final_mask, paste0(outfolder, gsub(" ","_", spp_name), '.tif'), overwrite=TRUE)
...
for(t in all_taxa){
  print(t)
  if(file.exists('data/pref_needs_info_btstgen7keep.csv')){
    prob_sp <- read.csv('data/pref_needs_info_btstgen7keep.csv', sep = ';')
  ...
  process_species <- function(spp) {
    ...
    tryCatch({
      ...
      if(!file.exists(paste0(outfolder, gsub(" ","_", spp ), '.tif'))){
        if(!spp %in% sp_li){
