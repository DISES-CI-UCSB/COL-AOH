...
run_analysis <- function(
  ...
  balance_specialist_generalist = TRUE, # balance specialist vs non specialist by taxa
  ...
  # dft_folder <- "aoh_results_randomCI_lv1_final"
  ...
  clusterExport(cl, c("habitat_info1", "ideam_lc_info", "ideam_lc_info_lv1", 
                      "cgls_lc_info", 
  ...
  clusterEvalQ(cl, {
    ...
    generate_matrix <- function(dat, landcover_colname, landcover_dataset, seed, cols){
      # dat, 'nvl_1_n', 'ideam', seed, colname_dataset
      tryCatch({
  ...
  run_single_iteration <- function(seed, df_all_info, df_basic_info, colname_dataset, balance_specialist_generalist) {
    ...
    tryCatch({
      ...
      this_ideam <- generate_matrix(dat, 'nvl_1_n', 'ideam', seed, colname_dataset)
...
# ================== Run =====================
dft_folder <- "results/aoh_results_randomCI_final_gen7_2022_lv1"
...
example_result <- run_analysis(
  ...
  balance_specialist_generalist = TRUE,
...
tert_df <- read.csv('results/aoh_results_randomCI_final_2022/btst_ideam_randomCI_gen5__noDesRockAA_0827_2025_tertiles.csv')
